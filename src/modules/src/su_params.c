...
float su_voltage_model_a = 0.341747;    // 전압-추력 모델 a
float su_voltage_model_b = -0.347462;    // 전압-추력 모델 b
float su_vbat_alpha      = 0.005f;       // 배터리 전압 LPF 알파
float su_dt_alpha        = 0.01f;        // dt 모니터링용 LPF 알파
float su_mob_alpha       = 0.5f;         // MOB 출력 LPF 알파   [안하겟단소리]
uint8_t su_zero_bias     = 0;            // MOB bias zero trigger
